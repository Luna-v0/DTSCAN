import numpy as np
from code.dtscan_fixed import DTSCAN, psr
...
def find_best_parameters(dataset_name, file_path, expected_clusters):
    ...
    threshold_values = np.linspace(-2.0, 0.0, 20)  # More granular search in the promising range
    ...
    for min_pts in min_pts_values:
        ...
        for area_th in threshold_values:
            for length_th in threshold_values:
                ...
                dtscan = DTSCAN(MinPts=min_pts, area_threshold=area_th, length_threshold=length_th)
                ...
                n_clusters_found = len(np.unique(pred_labels[pred_labels != -1]))
    ...
    if best_params_for_correct_clusters:
        print(f"Best Parameters Found for {dataset_name} (matching cluster count):")
        params = best_params_for_correct_clusters
        print(f"  MinPts: {params.get('min_pts', 'N/A')}")
        print(f"  Area Threshold: {params.get('area_threshold', 'N/A'):.3f}")
        print(f"  Length Threshold: {params.get('length_threshold', 'N/A'):.3f}")
        print(f"  Clusters found: {params.get('n_clusters_found', 'N/A')}")
        print(f"  Best PSR Score: {best_score_for_correct_clusters:.4f}")
    else:
        print(f"Could not find any parameter combination that resulted in {expected_clusters} clusters.")
...
if __name__ == "__main__":
    ...
    name = "S3"
    path = "datasets/S3.txt"
    print(f"\nPerforming focused search for {name}...")
    print(f"Expected for {name}: {expected_results[name]['clusters']} clusters, PSR {expected_results[name]['psr']:.3f}")
    find_best_parameters(name, path, expected_results[name]['clusters'])